<template>
    <div class="max-w-6xl mx-auto mb-8">
        <h1 class="text-2xl mt-8 mb-4 font-semibold">
            {{ genres ? `Games for Genre: "${genres.name}"` : 'Genre Not Found' }}
        </h1>
        ...
        <div v-else-if="!isLoading && games.length === 0" class="text-center py-4">
            <p>No games found for this genres.</p>
        </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        const genres = computed(() => store.genres.find(t => t.id === genreId.value));
        ...
        const fetchGamesByGenre = async () => {
            ...
            try {
                ...
                error.value = null;
                await store.fetchGamesByGenre(genreId.value, currentPage.value);
            } catch (err) {
                error.value = 'Failed to load games for this genres.';
            } finally {
        ...
        watch([() => route.params.id, currentPage], fetchGamesByGenre, { immediate: true });
