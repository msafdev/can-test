<template>
    <div class="max-w-6xl mx-auto mb-8">
        <h1 class="text-2xl mt-8 mb-4 font-semibold">
            {{ stores ? `Games for Store: "${stores.name}"` : 'Store Not Found' }}
        </h1>
        ...
        <div v-else-if="!isLoading && games.length === 0" class="text-center py-4">
            <p>No games found for this stores.</p>
        </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        const stores = computed(() => store.stores.find(t => t.id === storeId.value));
        const games = computed(() => store.games);
        const totalPages = computed(() => Math.ceil(store.totalGames / 12)); 

        
        const fetchGamesByStore = async () => {
            if (!storeId.value) return;

            try {
                isLoading.value = true;
                error.value = null;
                await store.fetchGamesByStore(storeId.value, currentPage.value);
            } catch (err) {
                error.value = 'Failed to load games for this stores.';
            } finally {
        ...
        watch([() => route.params.id, currentPage], fetchGamesByStore, { immediate: true });
